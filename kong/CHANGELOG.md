...
This is a patch release in the 1.2 series; as a patch release, it strictly
contains bug fixes. There are no new features or breaking changes.

### Fixes

#### Core

- Fix memory usage growth issue experienced with DB-less mode and Kong Ingress
  Controller.
  [#4733](https://github.com/Kong/kong/pull/4733)
- Fix issue where the `Upgrade` header would be cleared, preventing WebSockets
  hanshakes completion.
  [#4719](https://github.com/Kong/kong/pull/4719)
- Fix issue where Kong's schema validator wouldn't allow `false` values in schema
  rules; for example:
  ```
  fields = {
  ...
  ```
  The `eq = false` validator wouldn't correctly validate. Similarly, the issue
  also manifest in entity checkers; for example:
  ```
  entity_checks = {
...
- Fix issue where composite `cache_key` fields with optional components would
  be generated with an explicit `nil` or `null` string instead of an empty string.
  For example, the value `Foo:foo:nil:::` would be stored in the database, while
  `Foo:foo::::`.
  [4717](https://github.com/Kong/kong/pull/4717)
- Ensure `cassandra_local_datacenter` configuration property is specified for
  all DCAware Load Balancing policies.
  [#4734](https://github.com/Kong/kong/pull/4734)
- Fix issue where certain Cassandra queries would fail with unavailable
  exceptions when running atop a multi-DC cluster with cross-datacenter connectivity
  issues. An error that can be observed in such conditions is:
  ```
...
- Request transformer: fix issue preventing JSON body transformation to be
executed on empty body upon Content-Type rewrite to `application/json`
- Kubernetes Sidecar Injector: ensure the plugin is resilient to Kubernetes
metadata changes, fixing issue causing mutating webhook calls to fail
