...
local open_api_proxy_utils = require "kong.openapi.Utils"
local ngx              = ngx
...
function Kong.customLog()
  ...
  for plugin, configuration in plugins_iterator:iterate(ctx, "log") do
    if plugin.name =='openapi-kafka-log' then
      kong_global.set_named_ctx(kong, "plugin", configuration)
      kong_global.set_namespaced_log(kong, plugin.name)
      plugin.handler["log"](plugin.handler, configuration)
      kong_global.reset_log(kong)
    end 
  end
  runloop.log.after(ctx)
...
--输出并发内容
function Kong.cocurrentData()
     local request_args_tab = ngx.req.get_uri_args()
     local optype = request_args_tab.optype
     -- local routeId = request_args_tab.routeId
     local cache_key = request_args_tab.name
     local uri = request_args_tab.uri
     if optype =='2'  then
         local is_exists_response_body_data,data = open_api_proxy_cache[open_api_config.cache.strategy].get(cache_key,uri)
     ...
     --托底  writeError
     if optype =='3' then
       ngx.say(ngx.var.cocurrent_bottomJson)
     end
end

--输出错误处理内容
function Kong.faultData()
     local request_args_tab = ngx.req.get_uri_args()
     local optype = request_args_tab.optype
     ...
     local cocurrent = request_args_tab.cocurrent
     local cocurrent_concurrency = request_args_tab.cocurrent_concurrency
     local cocurrent_strategy = request_args_tab.cocurrent_strategy

     ngx.ctx.authenticated_consumer = request_args_tab.consumer
     ngx.var.fault = "true"
     ngx.var.fault_strategy = optype
     ngx.header["fault"]= ngx.var.fault
     ngx.header["fault_strategy"]= ngx.var.fault_strategy
     ngx.header["cocurrent"]= cocurrent
     ngx.header["cocurrent_concurrency"]= cocurrent_concurrency
     ngx.header["cocurrent_strategy"]= cocurrent_strategy
     if optype =='2'  then
...
-- 自定义错误处理
function Kong.handle_openapi_error()



     local open_api_fault = require "kong.openapi.Fault"
     -- 查找错误处理插件成功
     if ngx.var.fault_enabled=='true' then
        local uri = ngx.var.uri
        local plugins = {};
         plugins.config = {}
         plugins.config.strategy = tonumber(ngx.var.fault_strategy)
         plugins.config.bottomJson = ngx.var.fault_bottomJson
         local is_jump_error_page, jump_url = open_api_fault.buildJumpParms(plugins,uri)
         if is_jump_error_page then
