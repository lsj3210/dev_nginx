...
--        ngx.log(ngx.ERR,  "Redis_opt:increment()====2222===== res= ",res ,'   ttl=', ttl  )
        ...
        if not res then
            ngx.log(ngx.ERR, "increment_inc4444 error ==", err)
            return
        end
...
function Redis_opt:setStr(  redis_key ,val , timeout )


        local res, err = self.my_red:set(redis_key, val )
        if not res then
            ngx.log(ngx.ERR, "Redis_opt:increment_inc 设置过期时间1==",timeout,',err=',err )
--            self:close(my_red)
            return
        end

        -- 熔断打开 并且有超时时间 ，则设置过期时间
        if   val == 'open_circuit_breaker' and timeout ~= nil then
            ngx.log(ngx.ERR, "Redis_opt:increment_inc 设置过期时间2==",timeout )
            -- 过期时间为秒
            self.my_red:expire( redis_key, timeout )
        end
        
        if val == 'try_circuit_breaker' and timeout ~= nil then
            self.my_red:expire(redis_key, timeout)
        end

        if   val == '0' and timeout ~= nil then
--            ngx.log(ngx.ERR, "Redis_opt:increment_inc 设置过期时间3==",timeout )
            -- 过期时间为秒
            self.my_red:expire( redis_key, timeout )
        end
--      self:close(my_red)
...
function Redis_opt:getObj(redis_key)
--        local my_red = self:getRedis();
...
function Redis_opt:ttl(key)
  return self.my_red:ttl(key)
end
